...
class TAdresse:
    ...
    Attributes:
        ...
        ban_id: clé d'intéropérabilité de la BAN referençant l'adresse.
            (id dans l'API adresse d'etalab). Attention Il ne s'agit pas
            de l'identifiant BAN stable (banId)
        ban_id_ban_adresse: identifant BAN stable officiel de l'adresse.
            Le nom du champs est celui du format BAL 1.4 ( https://aitf-
            sig-topo.github.io/voies-
...
class Administratif:
    ...
    Attributes:
        ...
        diagnostiqueur:
        geolocalisation:
        consentement_proprietaire: A partir du 1er juillet 2024, lors de
            toutes ses interventions, le diagnostiqueur recueille le
            consentement de ses clients en vue de la transmission de
            leurs coordonnées à l’organisme de certification à des fins
            de contrôles, selon un modèle de formulaire fourni par les
            services du ministère chargé de la construction.
    ...
    class InformationConsentementProprietaire:
        """
        Attributes:
            nom_proprietaire: denomination ou raison sociale du
                propriétaire
            personne_morale: le propriétaire est-il une personne morale
            siren_proprietaire: siren du propriétaire, saisie
                obligatoire lorsqu'il s'agit d'une personne morale.
            telephone: numéro de téléphone du propriétaire
            mail: adresse mail du proprietaire
            label_adresse: libellé complet de l'adresse postale du bien
                saisi par le diagnostiqueur (FORMAT ATTENDU : Numéro de
                l’adresse dans la voie + Indice de répétition associé au
                numéro (par exemple bis, a…) + Nom de la voie en
                minuscules accentuées + Code postal du bureau de
                distribution de la voie + Nom officiel de la commune
                actuelle
            label_adresse_avec_complement: libellé complet d'adresse
                complète qui est l'adresse postale du bien précédé par
                l'ensemble des compléments d'adresses nécessaires à la
                bonne localisation du bien saisi par le diagnostiqueur
                (FORMAT ATTENDU : Compléments d'adresses + Numéro de
                l’adresse dans la voie + Indice de répétition associé au
                numéro (par exemple bis, a…) + Nom de la voie en
                minuscules accentuées + Code postal du bureau de
                distribution de la voie + Nom officiel de la commune
        ...
        class CaracteristiqueGenerale:
            ...
            Attributes:
                ...
                enum_methode_application_dpe_ter_id: methode
                    ...
                    neuf)
                enum_sous_modele_dpe_ter_id: sous modèle de DPE
